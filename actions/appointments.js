...
// VONAGE_PRIVATE_KEY env var should contain the full PEM key content
...
export async function bookAppointment(formData) {
  ...
  try {
    ...
    // Fetch the Clerk user's verified email address
    const clerkUser = await currentUser();
    const patientEmail = clerkUser?.emailAddresses?.[0]?.emailAddress ?? patient.email ?? null;

    // Create the appointment with the video session ID and patient email
    const appointment = await db.appointment.create({
      data: {
        ...
        patientEmail,          // ← store for reminder emails
        reminderSent: false,   // ← will be set true after first reminder
      },
    });
